...
import { Settings, X, LayoutGrid, ShoppingBag, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function App() {
  ...
  // Spotify now playing — for header marquee
  ...
  const widgetProps = {
    ...
    spotify: { onNowPlaying: setSpotifyNowPlaying },
    ...
    <div className="app">
      ...
      {spotifyNowPlaying?.name ? (
        <div style={{
          ...
          height: 28, display: 'flex', alignItems: 'center', overflow: 'hidden', position: 'relative', width: '100%',
        }}>
          <div style={{
            ...
            <span style={{ color: '#666', fontSize: '0.72rem', marginLeft: 80 }}>♫</span>
            <span style={{ color: '#e8e8ed', fontSize: '0.75rem', fontWeight: 500 }}>{spotifyNowPlaying.name}</span>
            <span style={{ color: '#666', fontSize: '0.72rem' }}>—</span>
            <span style={{ color: '#8888a0', fontSize: '0.72rem' }}>{spotifyNowPlaying.artist}</span>
          </div>
