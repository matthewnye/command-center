  ...
  const playTrack = async (track) => {
    if (isConfigured && track.uri) {
      ...
      if (result.status === 404 || result.status === 403) {
        setDeviceError(true); setTimeout(() => setDeviceError(false), 6000);
      } else {
  ...
  return (
    <div className="widget">
      ...
      <div className="widget-body">
        ...
        {/* Device error — inline, auto-dismisses */}
        {deviceError && (
          <div style={{ padding: '6px 10px', background: 'rgba(251,191,36,0.1)', borderRadius: 'var(--radius-sm)', fontSize: '0.75rem', color: '#fbbf24', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            <span>⚠️ No active device. Play something in Spotify first, then try again.</span>
          </div>
        )}
